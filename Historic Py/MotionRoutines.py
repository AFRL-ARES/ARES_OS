...
from BasicComm import sendCommand, getOutput, toSerBytes, getPos, getBedTemp, sendTCP
from BasicComm import setBedTemp, amaresConnect, amaresDisconnect, createLogFile, addToLog
from BasicComm import getRawPos, doBeep
...
def bedLeveling():
    try:
        if(config.status["homingDone"] == True):
            ...
            message = "Running auto bed leveling"
            addToLog(message)
            sendTCP(message)
            ...
            sendCommand("G0 X" + str(config.coords["initHome"][0]) +
                        " Y" + str(config.coords["initHome"][1]) +
                        " F6000", wait=True, log=True, listenTime=1.0)
...
def alignmentCamera():
    try:
        ...
        if(config.tool_vars["currentTool"] != "alignmentCamera"):
            ...
            sendCommand("G0 Z" + str(round(config.tool_vars["off"][2],3)) + " F500")
            sendCommand("G0 X" + str(round(config.tool_vars["off"][0],3)) + " Y" +
                        str(round(config.tool_vars["off"][1],3)) +" F3000", wait=True)
            if(config.status["homingDone"] == True):
                motionMode('absolute')
    ...
    try:
        ...
        if(config.tool_vars["currentTool"] != "depHead"):
            ...
            sendCommand("G0 X" + str(-1*round(config.tool_vars["off"][0],3)) + " Y" +
                        str(-1*round(config.tool_vars["off"][1],3)) + " F3000")
            sendCommand("G0 Z" + str(-1* round(config.tool_vars["off"][2],3)) + " F500", wait=True)
            if(config.status["homingDone"] == True):
                motionMode('absolute')
