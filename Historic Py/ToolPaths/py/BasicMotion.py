...
import pymsgbox
import copy
from BasicComm import sendCommand, getOutput, getPos, addToLog
...
def defineHome():
    try:
        ...
        addToLog("Dynamic Home changed to" + str(config.coords["dynHome"]))
...
def doBeep():
    ...
    try:
        config.ser_comm["taz"].write(b'M300 P45 S2500\n')
    except Exception as ex:
        addToLog("ERROR in BasicMotion/doBeep")
        addToLog("\t" + str(ex))
...
def runToolpath():
    ...
    try:
        ...
        while(len(config.tpGen["gcode"]) > 0):
            getOutput() # Checks (waits) for printer response and sets status accordingly
            if(len(config.tpGen["gcode"]) > 0) and (config.status["printerStatus"] == 'ready'):
                sendCommand(config.tpGen["gcode"][0])
                config.status["printerStatus"] = 'busy'
                del config.tpGen["gcode"][0] # removes first line
        getPos()
    except Exception as ex:
...
def checkMove(xyz):
    try:
        ...
        if((limW <= xyz[0] <= limE) and (limS <= xyz[1] <= limN) and (-1 <= xyz[2] <= 60)):
...
def doAlignOffset(coords):
    ...
    try:
        ...
        if(coords[0] != ''):
            coords[0] = float(coords[0]) + config.tool_vars["off"][0]
        if(coords[1] != ''):
            coords[1] = float(coords[1]) + config.tool_vars["off"][1]
        if(coords[2] != ''):
            coords[2] = float(coords[2]) + config.tool_vars["off"][2]
